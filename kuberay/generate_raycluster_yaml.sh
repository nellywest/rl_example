...
IMAGE_NAME="${DOCKERFILE_NAME%%.Dockerfile}:latest"
...
docker build --build-arg BASE_IMAGE=$BASE_IMAGE -f $DOCKERFILE_NAME -t "$DOCKER_USERNAME/$IMAGE_NAME" .
...
sed "s/{{DOCKER_USERNAME}}/$DOCKER_USERNAME/g; s/{{IMAGE_NAME}}/$IMAGE_NAME/g" kuberay/template_raycluster.yaml > kuberay/raycluster.yaml

# Check if the file was created successfully
if [ -s kuberay/raycluster.yaml ]; then
